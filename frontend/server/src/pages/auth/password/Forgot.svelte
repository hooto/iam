<script>
  ...
  async function onSubmit(/** @type {SubmitEvent} */ e) {
    e.preventDefault();
    alertMsg = "";
    ...
    try {
      const resp = await fetch(
        routePath + "/v2/auth/password/reset-ticket",
        {
          method: "POST",
  ...
  function goToSignIn(/** @type {Event} */ e) {
    e.preventDefault();
    ...
    {#if submitted}
      <div class="mb-4">
        ...
        <button
          ...
          onclick={() => {
            window.__navigate(routePath + "/auth/password/reset");
