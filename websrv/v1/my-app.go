...
func (c MyApp) InstListAction() {
	...
	q := base.NewQuerySet().From("ids_instance").Limit(myAppInstPageLimit)
	...
	if page > 1 {
		q.Offset(int64((page - 1) * myAppInstPageLimit))
	}
	...
	rsp.Meta.StartIndex = uint64((page - 1) * myAppInstPageLimit)
	rsp.Meta.ItemsPerList = uint64(myAppInstPageLimit)
