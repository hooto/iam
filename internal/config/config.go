...
func (it *ConfigCommon) reset() {
	...
	if it.InstanceID == "" {
		it.InstanceID = inauth.RandHexString(16)
	}
	...
	if len(it.AccessKeys) < 1 {
		key, _ := inauth.GenerateSecretKeyBase62(40)
		it.AccessKeys = append(it.AccessKeys, &inauth.AccessKey{
			Id:     inauth.RandHexString(8),
			Secret: key,
		})
	}
}
