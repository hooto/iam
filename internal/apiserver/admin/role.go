...
// RoleList returns the built-in roles, used to populate the role checkboxes
// in the admin user form.
func RoleList(ctx httpsrv.Ctx) error {
	...
	var rsp AdminRoleListResponse
	defer ctx.JSON(&rsp)

	offset := iamapi.NsRole("")
	cutset := append(offset, 0xff)

	if rs := data.Data.NewRanger(offset, cutset).SetLimit(1000).Exec(); rs.OK() {
